...
def execute(filters=None):
	...
	tot_bal_val = 0   
        ...
        for (parent, item, warehouse) in sorted(iwb_map):
                qty_dict = iwb_map[(parent, item, warehouse)]
                data.append([parent, 
	...
	if filters.get("warehouse"):
		...
		if warehouse_details:
			conditions += " and exists (select name from `tabWarehouse` wh \
				where wh.lft >= %s and wh.rgt <= %s and sl.warehouse = wh.name)"%(warehouse_details.lft,
...
def get_item_warehouse_map(filters):
	...
	sle = get_stock_ledger_entries(filters)
        frappe.msgprint(_(sle))
	for d in sle:
		frappe.msgprint(_(d))
                key = (d.parent, d.item_code, d.warehouse)
		...
		qty_dict = iwb_map[(d.parent, d.item_code, d.warehouse)]
